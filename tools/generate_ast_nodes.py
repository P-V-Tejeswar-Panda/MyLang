...
class FileContentsGenerator(Generator):
    ...
    def generate(self):
        ...
        if "headers" in self.spec:
            ret += HeaderGenerator(self.spec["headers"]).generate()            
            ret += "\n"*2
...
class ClassGenerator(Generator):
    ...
    def generate(self):
        ...
        fnGen = FuncGenerator(fnSpec)
        ret += indent(4, fnGen.generate())
        if "functions" in self.spec:
            for fnSpec in self.spec["functions"]:
                ret += indent(4, FuncGenerator(fnSpec).generate())
        ret += "};"
        return ret
...
class FuncGenerator(Generator):
    ...
    def generate(self):
        ...
        if "body" in self.spec:
            body = self.spec["body"]
        elif "returntype" not in self.spec and self.spec["name"][0] != "~" and "args" in self.spec:
            for arg in self.spec["args"]:
                body += "this->%s = %s;\n"%(arg[1], arg[1])
